...
6. 拥塞控制，全局性的过程，设计到全部主机
如何判断拥堵？拥堵了怎么办？
1）慢开始& 拥塞避免
如果一下子发送大量数据会马上引起拥塞，不知道的情况下慢慢增大
慢慢一点点地去探测当前网络的情况，判断拥堵是靠丢包来判断，但是TCP是基于网络出差错丢包的概率低，但是移动网络丢包可能的原因为很多，例如wifi信号弱，网络波动
一开始指数增长，到了阀值线性增长。
如何确定阀值？
只要发生了拥堵就把阀值降低为一半并且重新把拥塞窗口设置为初始值
3）快重传&快重传，预防网络拥堵
尽快重传未确认的报文，如果不使用快重传，就要等到超时才会重传，收到乱序的包马上回应前面的ack，收到三个ack即可认为丢包了马上重传
重复确认上一个未连续包
快重传后采用快重传方式，把阀值减半并且从阀值开始递增，中间只丢了几个包，很可能是没有发生拥塞，快恢复只是预防网络拥堵
